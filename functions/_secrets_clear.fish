function _secrets_clear -d "Clear cached secrets and environment variables"
    ...
    if set -q _flag_help
        ...
        printf "%s    secrets clear --quiet-footer # Clear without showing footer%s\n" $__SECRETS_COLOR_DIM $__SECRETS_COLOR_RESET
        return 0
    end
    ...
    _secrets_process "Clearing cached secrets..."
    ...
    if test -f "$cache_file"
        ...
        _secrets_info "No cache file found at: $(_secrets_dim $cache_file)"
    end

    printf "\n"
    _secrets_success "Success! Secrets cleared"

    # Footer hint (skip if called with --quiet-footer flag)
    if not set -q _flag_quiet_footer
        _secrets_hint "secrets refresh" "to reload secrets from 1Password"
    end
end
