...
- GitHub Actions runs `just lint` and `just test` on every push and pull request.
- The test suite uses mocked `op` behavior, so CI does not need access to a real 1Password vault.
- Releases are tag-driven. Pushing a semantic version tag such as `v0.1.0` triggers the release workflow.
- The release workflow reruns `just ci`, creates `.tar.gz` and `.zip` source archives, and publishes a GitHub Release with autogenerated notes.
